...
class RagChewProgressWidget(QWidget):
    ...
    def _create_actions_section(self) -> QGroupBox:
        ...
        report_btn.setToolTip("Generate a Rag Chew award report to submit to the award manager")
        report_btn.clicked.connect(self._open_award_report_dialog)
        layout.addWidget(report_btn)

        app_btn = QPushButton("Generate Application")
        app_btn.setToolTip("Generate a Rag Chew award application to submit to the award manager")
    ...
    def _open_award_report_dialog(self) -> None:
        ...
        try:
            ...
            dialog = AwardReportDialog(self.db, award_type='Rag Chew', parent=self)
            dialog.exec()
        except Exception as e:
    ...
    def _open_award_application_dialog(self) -> None:
        ...
        try:
            ...
            # Pre-select Rag Chew award
            dialog.award_combo.setCurrentText('Rag Chew')
            dialog.exec()
        except Exception as e:
