...
class VOACAPMUFFetcher:
    """Fetch and calculate Maximum Usable Frequency predictions with caching"""

    # 24-hour cache TTL for band predictions (86400 seconds)
    PREDICTION_CACHE_TTL_SECONDS = 86400
    ...
    def _generate_prediction_cache_key(
        ...
        """
        Generate a cache key for band predictions.
        Enables O(1) lookup for previously computed predictions instead of O(n) recalculation.

        Args:
            sfi: Solar Flux Index
            k_index: K-Index
            home_grid: Home grid square
        ...
        Returns:
            Cache key hash
        """
        # Use grid and space weather as cache key (location and propagation conditions)
        cache_input = f"{home_grid}_{sfi}_{k_index}"
        if latitude is not None:
            cache_input += f"_{latitude}"
        if longitude is not None:
            cache_input += f"_{longitude}"
        # Create hash for compact key
        return hashlib.md5(cache_input.encode()).hexdigest()

    def _get_cached_predictions(self, cache_key: str) -> Optional[Dict[str, MUFPrediction]]:
        """
        Get cached band predictions if available and fresh (within 24-hour TTL).

        Args:
            cache_key: Cache key generated by _generate_prediction_cache_key

        Returns:
            Cached predictions if available and fresh, None otherwise
        """
        if cache_key not in self.cached_predictions:
            return None

        predictions, timestamp = self.cached_predictions[cache_key]
        age_seconds = (datetime.now() - timestamp).total_seconds()

        if age_seconds > self.PREDICTION_CACHE_TTL_SECONDS:
            # Cache expired, remove it
            del self.cached_predictions[cache_key]
            logger.debug(f"MUF prediction cache expired after {age_seconds:.0f}s")
            return None

        logger.debug(f"MUF prediction cache hit (age: {age_seconds:.0f}s, TTL: {self.PREDICTION_CACHE_TTL_SECONDS}s)")
        return predictions

    def _cache_predictions(self, cache_key: str, predictions: Dict[str, MUFPrediction]) -> None:
        """
        Store band predictions in cache with timestamp.

        Args:
            cache_key: Cache key generated by _generate_prediction_cache_key
            predictions: Dictionary of band predictions to cache
        """
        self.cached_predictions[cache_key] = (predictions, datetime.now())
        logger.debug(f"Cached MUF predictions for {len(predictions)} bands (24-hour TTL)")
    ...
    def get_band_muf_predictions(
        ...
        Results cached for 24 hours for O(1) lookup performance.
        ...
        try:
            ...
            # Check if predictions are cached (24-hour TTL) - O(1) lookup
            cache_key = self._generate_prediction_cache_key(sfi, k_index, home_grid, latitude, longitude)
            cached = self._get_cached_predictions(cache_key)
            ...
            # Cache predictions for 24 hours to avoid recalculation - O(1) lookup on next call
            self._cache_predictions(cache_key, predictions)
