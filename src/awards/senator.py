...
class SenatorAward(AwardProgram):
    ...
    def calculate_progress(self, contacts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        unique_tribunes = set()
        # FIX N+1 QUERY: Load Tribune members once
        from src.database.models import TribuneeMember
        tribune_member_numbers = {m.skcc_number for m in session.query(TribuneeMember.skcc_number).all()}
        
        for contact in tribune_contacts:
            skcc_num = contact.skcc_number.strip()
            if skcc_num:
                base_number = extract_base_skcc_number(skcc_num)
                if base_number and base_number in tribune_member_numbers:
